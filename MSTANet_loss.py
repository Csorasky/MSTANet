...
class SimVP(Base_method):
    ...
    def second_derivative(self, pred_y, batch_y, tau=0.1, eps=1e-12):
        B, T, C = pred_y.shape[:3]
        if T <= 2:  return 0
        gap_pred_y = (pred_y[:, 1:] - pred_y[:, :-1]).reshape(B, T - 1, -1)
        gap_batch_y = (batch_y[:, 1:] - batch_y[:, :-1]).reshape(B, T - 1, -1)
        gap_pred_y = (gap_pred_y[:, 1:] - gap_pred_y[:, :-1]).reshape(B, T - 2, -1)
        gap_batch_y = (gap_batch_y[:, 1:] - gap_batch_y[:, :-1]).reshape(B, T - 2, -1)
        softmax_gap_p = F.softmax(gap_pred_y / tau, -1)
        softmax_gap_b = F.softmax(gap_batch_y / tau, -1)
        loss_gap = softmax_gap_p * \
                   torch.log(softmax_gap_p / (softmax_gap_b + eps) + eps)
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with self.amp_autocast():
                ...
                loss = self.criterion(pred_y, batch_y) + 0.5 * self.diff_div_reg(pred_y,
                                                                                 batch_y) + 0.5 * self.second_derivative(
                    pred_y, batch_y)
